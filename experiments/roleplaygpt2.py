#!/usr/bin/env python3
"""
Role-Playing Jailbreak Data Collection Script with GPT-2
...
# ===================== CONFIGURATION =====================
MODEL_NAME = "/root/autodl-tmp/models/gpt2"   # 根据实际存放位置调整
DEVICE = "cuda:0" if torch.cuda.is_available() else "cpu"
MAX_NEW_TOKENS = 150
TEMPERATURE = 0.7
DATASET_CSV = "data/advbench/harmful_behaviors.csv"
NUM_SAMPLES = 15
OUTPUT_FILE = f"roleplay_data_{MODEL_NAME.replace('/', '_')}.jsonl"  # 包含模型名，扩展名改为 .jsonl
...
def main():
    ...
    # 4. 保存结果（文件名为 jsonl，内容仍为 JSON 对象）
